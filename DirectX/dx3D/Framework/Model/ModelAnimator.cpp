	...
	: shader(shader)
{
	...
	DistanceCollider = new Collider(new Transform());
	HitDistanceCollider = new Collider(new Transform());
	LongCollider = new Collider(new Transform());
...
void ModelAnimator::OneFrame(int i, bool check)
{
	if (check)
	{
		if (death[i] > 11) return;
		death[i] += Time::Delta();
	}
...
void ModelAnimator::PlayClip(UINT instance, UINT clip, float speed, float takeTime , bool check)
{
	if (check)
	{
...
void ModelAnimator::CreateTexture()
{
	...
	//Create Texture
	{
		...
		UINT pageSize = MAX_MODEL_TRANSFORMS * 16 * 4 * MAX_MODEL_KEYFRAMES;
