...
VertexTerrain VS(VertexTextrueNormal input)
{
...
TerrainOutput VS_TerrainShadow(VertexTextrueNormal input)
{
	...
	output.Normal = mul(input.Normal, (float3x3) World);
	output.Tangent = WorldTangent(input.Tangent); // tagent 어캐함 내가 추가함 거임
	output.Uv = input.Uv;
...
//Depth
//------------------------------------------------------------------------------------------------------
VertexTerrain VS_Depth_Terrain(VertexTextrueNormal input)
{
	VertexTerrain output;
	output.Position = mul(input.Position, World);
	output.wPosition = output.Position.xyz;
