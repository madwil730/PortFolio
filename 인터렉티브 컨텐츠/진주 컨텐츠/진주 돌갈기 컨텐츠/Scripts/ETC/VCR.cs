...
public class VCR : MonoBehaviour
{
	...
	//void Play(E_ButtonType type)
	//{
	//	IsPlaying = true;
	//	string fileName = GetVideoFileName(type);
	//	vcr.OnOpenVideoFile(fileName);
	...
	{
		LoadingPlayer.OpenVideoFromFile(_location, System.IO.Path.Combine(_folder, _videoFiles[num]), _AutoStartToggle.isOn);
	}
